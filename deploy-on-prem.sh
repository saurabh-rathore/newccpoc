...
if [ ! -f /etc/kubernetes/admin.conf ]; then
    ...
    # Configure kubectl for root and sudo user
    mkdir -p $HOME/.kube
...
# --- Step 5: Setup On-Premise Components (GPU, LoadBalancer, Registry) ---
...
spec:
  addresses:
  - ${HOST_IP}-${HOST_IP} # Use the host's IP for the load balancer
EOF
...
# --- Step 6: Build and Push Images to Local Registry ---
...
# Update manifests to use local registry and remove AWS-specific storage class
for FILE in kubernetes/*.yaml; do
    sed -e "s|<your_docker_registry>/|localhost:5000/|g" \
        -e "s|storageClassName: \"gp2\"|# storageClassName: \"gp2\" # Using default storage class|g" \
